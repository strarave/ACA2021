	...
	\part{Parallelism}
		...
		\section{Dynamic Scheduling}
			...
			\subsection{Scoreboard - Dynamic Scheduler Implementation}
				...
				\begin{enumerate}
					\item The issue stage decodes the instruction and detects the hazards. Instructions are issued in program order. Stalling this phase is used to avoid WAW and structural hazards. If no hazards are detected, the Read Operands stage is entered
					\item Due to the fact that there's no possible forwarding in this pipeline, scoreboard hardware needs to tell each FU when it can read from registers. This is the stage to be stalled in order to avoid RAW hazards.
			 		\item Execution stage: the FU gets the operands when the scoreboard enables the Read Operands stage "right behind" the exe stage, and this stage notifies the scoreboard when the result is ready. 
				...
				\subsubsection{Tomasulo data structures}
			 	...
			 	\subsubsection{Three stages Tomasulo architecture}
					...
					\paragraph{ISSUE stage}
						Get the instruction from the queue and if a RS is ready, \emph{set the right values of it}. WAR and WAR conflicts are handled in this stage.
					
					\paragraph{EXECUTION stage}
						There's no "get operands" stage, because the stalls are all condensed in the ISSUE stage, and because if an operand is not ready \emph{it will arrive from the shared data bus}.
						
					\paragraph{WRITE stage}
						In this phase the result of the FU computations are broadcasted on the common data bus and stored in the RS that requested it and in the Store Buffers, in order to be stored in memory or in the RF.
\end{document}
