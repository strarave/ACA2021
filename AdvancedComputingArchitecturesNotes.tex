	...
	\part{Parallelism}
		\section{Instruction Level Parallelism}
			...
			\subsection{VLIW}
				...
				\end{itemize}
				The compiler now has to arrange operations in these batches (the VLIW instructions) that (due to their dimension) justify the name "Very Long Instruction". So basically the pipeline architecture isn't changed, the machine loop "IF ID EXE MEM WB" is not changed, but \emph{every single block of the pipeline is replicated} and working simultaneously. MAIN ASSUMPTION: the code to be executed is free from any kind of dependences and shit like that. To enforce this assumtpion/condition, the compiler must be made more complex in order to \emph{find and neutralize} data dependencies that can create hazards. This approach is called Static Scheduling because it exploits software components to schedule at best the instructions.\\
