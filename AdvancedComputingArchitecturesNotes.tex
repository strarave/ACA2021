...
\begin{document}
	...
	\part{Multiple Cores}
		\section{How to Handle Explicit Parallelism}
			...
			\end{enumerate}
			Obviously, switching between two processes is more heavy than switching between two threads.
			
			\subsection{Multithread Single Core Architectures}
				...
				\begin{enumerate}
					\item Fine grain parallelism: each instruction is followed by another thread instruction. This hides short and long stalls, because a stalled thread "leaves the pipeline free" for another thread. 
				...
				\end{enumerate}
				Can ILP and TLP (Thread Level Parallelism) live together on an architecture? Yes; \emph{simultaneous multithreading}. We can exploit a superscalar architecture to fetch instruction \emph{from different indipendent threads} when loading instructions. 
